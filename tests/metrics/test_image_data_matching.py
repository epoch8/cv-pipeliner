...
true_bbox_data_banana = BboxData(  # detection: FN, pipeline: FN for banana
    image_path=image_path, xmin=296, ymin=104, xmax=1190, ymax=650, label="banana"
)
pred_bbox_data_banana_part1 = BboxData(  # detection: FP, pipeline: FP for banana
    image_path=image_path, xmin=314, ymin=122, xmax=501, ymax=314, label="banana"
)
...
pred_bbox_data_extra_bbox2 = BboxData(  # detection: FP, pipeline: FP for apple
    image_path=image_path, xmin=1602, ymin=788, xmax=1835, ymax=1013, label="apple"
)
...
def test_image_data_matching_detection():
    ...
    image_data_matching = ImageDataMatching(
        true_image_data=true_image_data, pred_image_data=pred_image_data, minimum_iou=minimum_iou
    )
    ...
    bbox_data_matching_banana_part1 = image_data_matching.find_bbox_data_matching(
        pred_bbox_data_banana_part1, tag="pred"
    )
    bbox_data_matching_banana_part2 = image_data_matching.find_bbox_data_matching(
        pred_bbox_data_banana_part2, tag="pred"
    )
    ...
    # Top apple tests
    bbox_data_matching_top_apple = image_data_matching.find_bbox_data_matching(true_bbox_data_top_apple, tag="true")
    ...
    # Bottom apple tests
    bbox_data_matching_bottom_apple_true = image_data_matching.find_bbox_data_matching(
        true_bbox_data_bottom_apple, tag="true"
    )
    bbox_data_matching_bottom_apple_pred = image_data_matching.find_bbox_data_matching(
        pred_bbox_data_bottom_apple, tag="pred"
    )
    ...
    bbox_data_matching_mango_outside = image_data_matching.find_bbox_data_matching(
        pred_bbox_data_mango_outside, tag="pred"
    )
    bbox_data_matching_mango_inside = image_data_matching.find_bbox_data_matching(
        pred_bbox_data_mango_inside, tag="pred"
    )
    ...
    # Extra bboxes tests
    bbox_data_matching_extra_bbox1 = image_data_matching.find_bbox_data_matching(pred_bbox_data_extra_bbox1, tag="pred")
    bbox_data_matching_extra_bbox2 = image_data_matching.find_bbox_data_matching(pred_bbox_data_extra_bbox2, tag="pred")
...
def test_image_data_matching_pipeline():
    ...
    for tag in [
        tag_default,
        tag_with_extra_bbox_label_other,
        tag_known_banana_apple,
        ...
        # Extra bboxes tests
        bbox_data_matching_extra_bbox1 = image_data_matching.find_bbox_data_matching(
            pred_bbox_data_extra_bbox1, tag="pred"
        )
        bbox_data_matching_extra_bbox2 = image_data_matching.find_bbox_data_matching(
            pred_bbox_data_extra_bbox2, tag="pred"
        )
        ...
        for label in [None, "apple", "banana", "mango", "other"]:
            ...
            ).save(test_dir / f"df_metrics_pipeline_{tag=}_{label=}.png")
        dfi.export(
