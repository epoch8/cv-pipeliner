    ...
    def visualize(
        ...
        if use_all_data:
            ...
            if self.inferencer is not None and show_TP_FP:
                ...
                n_true_labels = [
                    np.array([bbox_data.label for bbox_data in bboxes_data]) for bboxes_data in n_bboxes_data
                ]
                n_pred_labels = [
                    np.array([bbox_data.label for bbox_data in bboxes_data]) for bboxes_data in n_pred_bboxes_data
                ]
                n_TP = [
                    np.sum(true_labels == pred_labels) for true_labels, pred_labels in zip(n_true_labels, n_pred_labels)
                ]
                n_FP = [
                    np.sum(true_labels != pred_labels) for true_labels, pred_labels in zip(n_true_labels, n_pred_labels)
                ]
        ...
        def display_fn(i):
            if self.i is None or i != self.i:
                ...
                if self.inferencer is not None:
                    n_bbox_data_gen = BatchGeneratorBboxData(
                        [self.true_bboxes_data], batch_size=batch_size, use_not_caught_elements_as_last_batch=True
                    )
                ...
                class_names = sorted(
                    list(set(["all"] + list(true_labels))), key=lambda x: label_to_count[x], reverse=True
                )
                if self.inferencer is not None and show_TP_FP:
                    class_names_visible = [
                    ...
                    class_names_visible = [
                        f"{class_name} [{label_to_count[class_name]} total]" for class_name in class_names
                    ]
