...
def visualize_bboxes_data(
    ...
    true_labels = np.array([bbox_data.label for bbox_data in bboxes_data])
    ...
    if pred_bboxes_data is not None and type_only is not None:
        if type_only == "TP":
            data_by_class_name = data_by_class_name & (true_labels == pred_labels)
        elif type_only == "FP":
            data_by_class_name = data_by_class_name & (true_labels != pred_labels)
    ...
    if use_random:
        idxs = np.random.choice(data_by_class_name_idxs, size=size, replace=False)
    else:
    ...
    if size >= 10:
        figsize += 20
    fig, axes = plt.subplots(int(np.ceil(size / 5)), 5, figsize=(20, figsize + size // 5))
