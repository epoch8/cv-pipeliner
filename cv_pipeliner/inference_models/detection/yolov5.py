...
class YOLOv5_DetectionModel(DetectionModel):
    def _load_yolov5_model(self, model_spec: YOLOv5_ModelSpec):
        ...
        self.model = torch.hub.load(
            "ultralytics/yolov5", "custom", path=str(model_path_tmp), force_reload=model_spec.force_reload
        )
        if model_spec.device is not None:
            self.model = self.model.to(model_spec.device)
        temp_file.close()

    def __init__(self, model_spec: Union[YOLOv5_ModelSpec, YOLOv5_TFLite_ModelSpec]):
