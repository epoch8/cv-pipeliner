...
def classification_interactive_work(
    ...
    classification_visualizer.visualize(n_bboxes_data=n_bboxes_data, use_all_data=use_all_data, show_TP_FP=True)
...
def concat_classifications_reports_datas(
    ...
    classification_report_data = ClassificationReportData(
        df_classification_metrics=df_classification_metrics, tops_n=tops_n, collect_the_rest=False
    )
...
class ClassificationReporter(Reporter):
    def _get_markdowns(self, classification_report_data: ClassificationReportData, tags: List[str] = None) -> List[str]:
    ...
    def _get_codes(self, tags: List[str]) -> List[str]:
        codes = []
        codes.append(
            """
import os
...
classification_interactive_work(
    ...
    def _inference_classification_and_get_metrics(
        ...
        bboxes_data_gen = BatchGeneratorBboxData(
            n_true_bboxes_data, batch_size=batch_size, use_not_caught_elements_as_last_batch=True
        )
    ...
    def report(
        ...
        for model_spec in models_specs:
            if hasattr(model_spec, "preprocess_input"):
                assert isinstance(model_spec.preprocess_input, str) or isinstance(model_spec.preprocess_input, Path)
        ...
        for model_spec, tag in zip(models_specs, tags):
            ...
            classifications_reports_datas.append(
                ClassificationReportData(
                    df_classification_metrics=tag_df_classification_metrics, tops_n=tops_n, tag=tag
                )
            )
    ...
    def report_on_predictions(
        ...
        classifications_reports_datas = [
            ClassificationReportData(df_classification_metrics=tag_df_classification_metrics, tops_n=tops_n, tag=tag)
        ]
