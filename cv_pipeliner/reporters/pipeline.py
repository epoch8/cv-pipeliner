...
def pipeline_interactive_work(
    directory: Union[str, Path], tag: str, detection_score_threshold: float, minimum_iou: float, extra_bbox_label: str
):
...
class PipelineReportData:
    ...
    def __init__(
        ...
        self.df_pipeline_metrics_short = self._get_df_pipeline_metrics_short(df_pipeline_metrics=df_pipeline_metrics)
        ...
        self.df_incorrect_preds = self._get_df_incorrect_preds(
            df_detection_metrics=df_detection_metrics, df_pipeline_metrics=df_pipeline_metrics
        )
...
def concat_pipelines_reports_datas(
    pipelines_reports_datas: List[PipelineReportData], compare_tag: str = None
) -> PipelineReportData:
    ...
    df_detection_metrics = transpose_columns_and_write_diffs_to_df_with_tags(
        df_with_tags=pd.concat(
            [tag_pipeline_report_data.df_detection_metrics for tag_pipeline_report_data in pipelines_reports_datas],
            axis=1,
        ),
    ...
    df_pipeline_metrics = transpose_columns_and_write_diffs_to_df_with_tags(
        df_with_tags=pd.concat(
            [tag_pipeline_report_data.df_pipeline_metrics for tag_pipeline_report_data in pipelines_reports_datas],
            axis=1,
            join="outer",
    ...
    pipeline_report_data = PipelineReportData(
        df_detection_metrics=df_detection_metrics, df_pipeline_metrics=df_pipeline_metrics, collect_the_rest=False
    )
    ...
    pipeline_report_data.df_correct_preds = transpose_columns_and_write_diffs_to_df_with_tags(
        df_with_tags=pd.concat(
            [tag_pipeline_report_data.df_correct_preds for tag_pipeline_report_data in pipelines_reports_datas], axis=1
        ),
        columns=["value"],
        tags=tags,
        compare_tag=compare_tag,
...
class PipelineReporter(Reporter):
    def _get_markdowns(self, pipeline_report_data: PipelineReportData, tags: List[str] = None) -> List[str]:
...
pipeline_interactive_work(
    ...
    def _inference_pipeline_and_get_metrics(
        ...
        raw_pred_images_data = inferencer.predict(images_data_gen, detection_score_threshold=0.0)
        pred_images_data = [
            ImageData(
    ...
    def report(
        ...
        for model_spec, tag, detection_score_threshold, extra_bbox_label in zip(
            ...
            pipelines_reports_datas.append(
                PipelineReportData(
                    df_detection_metrics=tag_df_detection_metrics, df_pipeline_metrics=tag_df_pipeline_metrics, tag=tag
                )
            )

        pipeline_report_data = concat_pipelines_reports_datas(
            pipelines_reports_datas=pipelines_reports_datas, compare_tag=compare_tag
        )
        markdowns = self._get_markdowns(pipeline_report_data=pipeline_report_data, tags=tags)
    ...
    def report_on_predictions(
        ...
        markdowns = self._get_markdowns(pipeline_report_data=pipeline_report_data, tags=[tag])
        self._save_report(
            models_specs=[None],
            tags=[tag],
            output_directory=output_directory,
