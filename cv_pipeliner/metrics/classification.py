...
def _add_metrics_to_dict(
    ...
    if count_mean_expected_steps:
        ...
        mean_expected_steps_supports = [classification_metrics[class_name]["support"] for class_name in not_nan_labels]
        ...
        weighted_average_mean_expected_steps = (
            np.average(mean_expected_steps, weights=mean_expected_steps_supports)
            if len(mean_expected_steps) > 0
            else np.nan
        )
        classification_metrics[f"{prefix_caption}macro_average{postfix_caption}"][
            "mean_expected_steps"
    ...
    for top_n in tops_n:
        ...
        weighted_average_precision_top_n = (
            np.average(precisions_top_n, weights=supports) if len(precisions_top_n) > 0 else np.nan
        )
        macro_average_recall_top_n = np.average(recalls_top_n) if len(recalls_top_n) > 0 else np.nan
        weighted_average_recall_top_n = (
            np.average(recalls_top_n, weights=supports) if len(recalls_top_n) > 0 else np.nan
        )
        ...
        weighted_average_f1_score_top_n = (
            np.average(f1_score_top_n, weights=supports) if len(f1_score_top_n) > 0 else np.nan
        )
        classification_metrics[f"{prefix_caption}macro_average{postfix_caption}"][
            f"precision@{top_n}"
...
def get_precision_and_recall_top_n(
    true_labels: List[str], pred_labels_top_n: List[List[str]], label: str, top_n: int
) -> Tuple[int, int]:
...
def get_df_classification_metrics(
    ...
    pred_labels_top_n = np.array(
        [
            bbox_data.labels_top_n if bbox_data.labels_top_n is not None else [bbox_data.label]
    ...
    n_pred_labels_top_n = [
        np.array(
            [
                pred_bbox_data.labels_top_n if pred_bbox_data.labels_top_n is not None else [pred_bbox_data.label]
    ...
    MMM = multilabel_confusion_matrix(y_true=true_labels, y_pred=pred_labels, labels=all_class_names)
        ...
        if max(tops_n) > 1:
            for top_n in tops_n:
                ...
                precision_top_n, recall_top_n = get_precision_and_recall_top_n(
                    true_labels=true_labels, pred_labels_top_n=pred_labels_top_n, label=class_name, top_n=top_n
                )
                ...
                classification_metrics[class_name][f"f1_score@{top_n}"] = (2 * precision_top_n * recall_top_n) / max(
                    precision_top_n + recall_top_n, 1e-6
                )
    ...
    df_classification_metrics_columns = (
        ["support"]
        + df_classification_metrics_MES_column
        + ["precision", "recall", "f1_score", "value"]
        + [
            item
            for sublist in [[f"precision@{top_n}", f"recall@{top_n}"] for top_n in tops_n if top_n > 1]
            for item in sublist
        ]
        + ["TP", "FP", "FN"]
    )
    ...
    if known_class_names is not None:
        df_classification_metrics.loc[all_class_names, "known"] = df_classification_metrics.loc[
            all_class_names
        ].index.isin(known_class_names)
        df_classification_metrics.loc[all_class_names, "pseudo"] = df_classification_metrics.loc[
            all_class_names
        ].index.isin(pseudo_class_names)
