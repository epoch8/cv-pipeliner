...
class TensorFlow_KeypointsRegressorModelSpec(KeypointsRegressorModelSpec):
    ...
    @property
    def runtime_cls(self) -> Type["TensorFlowKeypointsRegressorRuntime"]:
        from cv_pipeliner.inferencers.keypoints_regressor.tensorflow import (
            TensorFlowKeypointsRegressorRuntime,
        )

        return TensorFlowKeypointsRegressorRuntime
...
class TensorFlowKeypointsRegressorRuntime(KeypointsRegressorRuntime):
    def _load_tensorflow_KeypointsRegressor_model_spec(self, model_spec: TensorFlow_KeypointsRegressorModelSpec):
        ...
        if model_spec.saved_model_type in ["tf.keras", "tf.saved_model", "tflite", "tflite_one_image_per_batch"]:
            ...
            raise ValueError(
                "TensorFlowKeypointsRegressorRuntime got unknown saved_model_type "
    ...
    def __init__(
        ...
        if isinstance(model_spec, TensorFlow_KeypointsRegressorModelSpec):
        ...
        elif isinstance(model_spec, TensorFlow_KeypointsRegressorModelSpec_TFServing):
            ...
            raise ValueError(
                f"TensorFlowKeypointsRegressorRuntime got unknown KeypointsRegressorModelSpec: {type(model_spec)}"
            )
    ...
    def _raw_predict_tensorflow(self, images: np.ndarray):
        ...
        if self.model_spec.saved_model_type == "tf.saved_model":
            ...
            for image in images:
                ...
                image_tensor = tf.convert_to_tensor([image], dtype=self.input_dtype)
