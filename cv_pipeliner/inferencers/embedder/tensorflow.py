...
class TensorFlow_EmbedderModelSpec(EmbedderModelSpec):
    ...
    @property
    def runtime_cls(self) -> Type["TensorFlowEmbedderRuntime"]:
        from cv_pipeliner.inferencers.embedder.tensorflow import (
            TensorFlowEmbedderRuntime,
        )

        return TensorFlowEmbedderRuntime
...
class TensorFlowEmbedderRuntime(EmbedderRuntime):
    def _load_tensorflow_embedder_model_spec(self, model_spec: TensorFlow_EmbedderModelSpec):
        ...
        if model_spec.saved_model_type in ["tf.keras", "tf.saved_model", "tflite", "tflite_one_image_per_batch"]:
            ...
            raise ValueError(
                "TensorFlowEmbedderRuntime got unknown saved_model_type "
                f"in TensorFlow_EmbedderModelSpec: {model_spec.saved_model_type}"
            )

    def __init__(
        self,
        model_spec: Union[TensorFlow_EmbedderModelSpec, TensorFlow_EmbedderModelSpec_TFServing],
    ):
        ...
        if isinstance(model_spec, TensorFlow_EmbedderModelSpec):
            ...
            raise ValueError(f"TensorFlowEmbedderRuntime got unknown EmbedderModelSpec: {type(model_spec)}")
