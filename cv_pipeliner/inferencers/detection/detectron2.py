...
class Detectron2_ModelSpec(DetectionModelSpec):
    ...
    @property
    def runtime_cls(self) -> Type["Detectron2Runtime"]:
        from cv_pipeliner.inferencers.detection.detectron2 import (
            Detectron2Runtime,
        )

        return Detectron2Runtime
...
class Detectron2Runtime(DetectionRuntime):
    ...
    def __init__(self, model_spec: Detectron2_ModelSpec):
        ...
        if isinstance(model_spec, Detectron2_ModelSpec):
            ...
            raise ValueError(f"{Detectron2Runtime.__name__} got unknown DetectionModelSpec: {type(model_spec)}")
