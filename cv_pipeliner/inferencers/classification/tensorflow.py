...
class TensorFlow_ClassificationModelSpec(ClassificationModelSpec):
    ...
    @property
    def runtime_cls(self) -> Type["TensorFlowClassificationRuntime"]:
        from cv_pipeliner.inferencers.classification.tensorflow import (
            TensorFlowClassificationRuntime,
        )

        return TensorFlowClassificationRuntime
...
class TensorFlowClassificationRuntime(ClassificationRuntime):
    def _load_tensorflow_classification_model_spec(self, model_spec: TensorFlow_ClassificationModelSpec):
            ...
            try:
                if model_spec.saved_model_type in "tf.keras":
                    self.model = tf.keras.models.load_model(
                        str(model_path), compile=False, custom_objects=model_spec.custom_objects
                    )
    ...
    def __init__(
        self,
        model_spec: Union[TensorFlow_ClassificationModelSpec, TensorFlow_ClassificationModelSpec_TFServing],
    ):
        ...
        if isinstance(model_spec, TensorFlow_ClassificationModelSpec):
            ...
            raise ValueError(f"TensorFlowClassificationRuntime got unknown ClassificationModelSpec: {type(model_spec)}")
    ...
    def predict(self, input: ClassificationInput, top_n: int = 1) -> ClassificationOutput:
        ...
        id_to_class_names_repeated = np.repeat(a=self.id_to_class_name[None, ...], repeats=len(input), axis=0)
