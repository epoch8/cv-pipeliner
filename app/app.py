...
from model import name_to_detection_description, name_to_classification_description, load_pipeline_inferencer
...
detection_model_name = st.selectbox(
    label='Detection',
    options=[None] + [name for name in name_to_detection_description]
)
classification_model_name = st.selectbox(
    label='Classification',
    options=[None] + [name for name in name_to_classification_description]
)
if detection_model_name is not None and classification_model_name is not None:
    data_load_state.text("Loading models...")
    detection_model_description = name_to_detection_description[detection_model_name]
    ...
    def inference_one_image(uploaded_file):
        ...
        pred_image_data = pipeline_inferencer.predict(
            image_data_gen,
            detection_score_threshold=detection_model_description.score_threshold,
