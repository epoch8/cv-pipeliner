...
from cv_pipeliner.core.inference_model import (
    ModelSpec, InferenceModel
)

from .dataclass_dataset import DataClassDataSet


class InferenceModel_Dataset(AbstractDataSet):
    def __init__(
        self,
    ...
    def _load(self) -> InferenceModel:
        model_spec = self.model_spec_dataset.load()
        return model_spec.load()

    def _save(self, model: InferenceModel) -> None:
        self.model_spec_dataset._save(model.model_spec)
